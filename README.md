...
Change the Fencing_Camera source to your camera. Check that the Putty Sources are referencing the window displaying Putty. I have found that it helps if Putty is fullscreen in the background. The OBS window should also be active for the hotkeys to be detected by OBS. The hotkeys are assigned in settings using [HID Key Input Tool](https://github.com/BenKohn2004/Favero_Overlay/tree/main/Micro_Pro_Favero_HID_Key_Input_Tool). Program is loaded onto a Micro Pro and then sends a new keystroke each second to the computer, which can be captured in the OBS Settings Hotkeys. [Here](https://github.com/BenKohn2004/Favero_Overlay/blob/main/HotKey%20Settings%20Screen%20Shot.JPG) is an example of how the hotkeys will look when assigned in OBS. The OBS screen has to be active for the hotkeys to toggle the visibility of the lights.
